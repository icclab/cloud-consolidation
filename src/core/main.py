...
def run(env, description, ccc=1.0, ccs=1.0):
    '''
    Runs both phases of the consolidation algorithm
    for given environment (env) with a custom parametrization
    of capacity coefficientes for both the consolidation
    phase (ccc) and the spliting phase (ccs)
    '''
    ...
    actions = cc_mod.consolidate(env, ccc)
    env_mod.print_env_available_res(env)
    env_mod.print_env_available_res_rel(env)
...
def main():
    ...
    ccc = 1.0
    ccs = 1.0
    run(copy.deepcopy(env), ('%s;ccs:%s,ccc:%s' % (description, ccs, ccc)),
        ccc=ccc, ccs=ccs)
