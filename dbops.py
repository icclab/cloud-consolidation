...
def get_ts_ceilometer():
    ...
    try:
        ts = int((datetime.datetime.strptime(str(samples[0]._info['timestamp']), '%Y-%m-%dT%H:%M:%S.%f')).strftime('%s'))
...
def save_cons_rec(ts, simulated = False):
    ...
    with closing(sqlite3.connect(DATABASE)) as db:
        db.execute('insert into consolidations(timestamp, username, url, tenant_name)\
                        values(?, ?, ?, ?);', (
                            ts, username, url, tn,))
        db.commit()
...
def save_environment(cons_id, ts, env, phase, actions=[]):
    with closing(sqlite3.connect(DATABASE)) as db:
        for action in actions:
            db.execute('insert into actions(consolidation_id, vm_id, src_host, dst_host, phase)\
                            values(?, ?, ?, ?, ?);', (
                                cons_id, action["vm_id"], action["src"], action["dst"], phase,))

        for host in env["hosts"]:
            db.execute('insert into hypervisors(\
			    ...
			    consumption_idle, consumption_max)\
                            values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);', (
                                cons_id, host["id"], host["cpu"], host["util"], host["capacity"],
                                host["ram_used"], host["ram_capacity"], host["power_consumption"],
                                phase, host["host_ip"], host["no_vms"], ts, host["consumption_idle"],
